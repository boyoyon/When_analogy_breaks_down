...
def getPos(x,z):
    ...
    x0 = np.cos(np.deg2rad(x))
    y0 = np.sin(np.deg2rad(x))
...
def main():
    ...
    while not fTerminate:
        ...
        if fUpdate:
            ...
            cv2.putText(clone, text, FONT_POS, FONT, FONT_SIZE, GREEN, 2)
            ...
            cv2.line(clone, ((POS[2][0]+POS[3][0])//2,(POS[2][1]+POS[3][1])//2),CENTER,RED, 5)
